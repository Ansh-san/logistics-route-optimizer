// routingUtils.js  (server-side copy)
// Identical algorithms to the frontend version, but WITHOUT the module-level
// reference-equality cache (_cachedNodes / _cachedEdges / _cachedAdjacency)
// which is unsafe in a shared Node process where successive requests may
// supply different edge arrays that happen to share the same object identity.
...
// ----- multi-stop route optimization (open-path TSP) -----
...
/**
 * Solves an open-path TSP: visit all stopIds starting from startId,
...
export function solveMultiStopRoute(nodes, edges, startId, stopIds) {
  ...
  // Pre-compute shortest path between every relevant pair
  ...
  // Flatten legs into one continuous path
